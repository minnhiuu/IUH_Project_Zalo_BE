...
public class UserServiceImpl implements UserService {
    ...
    public UserResponse createUser(UserCreateRequest request) {
        ...
        // Generate userId from the MongoDB generated id
        // Since we need the userId to be set before saving, we'll use the id field
        // The id will be generated by MongoDB and we'll copy it to userId after save
        
        // Initialize default user settings if not already set
        if (user.getUserSetting() == null) {
            user.setUserSetting(createDefaultUserSettings());
            log.info("Default user settings initialized for new user");
        }
        
        user = userRepository.save(user);
        
        // Set userId to be the same as the MongoDB generated id
        if (user.getUserId() == null) {
            user.setUserId(user.getId());
            user = userRepository.save(user);
            log.info("UserId set to: {}", user.getUserId());
        }
        
        log.info("User created successfully with id: {} and userId: {} with default settings", user.getId(), user.getUserId());
        return userMapper.toUserResponse(user);
    }
    
    /**
     * Creates default user settings with all nested objects initialized
     */
    private UserSetting createDefaultUserSettings() {
        return UserSetting.builder()
                .generalSettings(new UserSetting.GeneralSettings())
                .securitySettings(new UserSetting.SecuritySettings())
                .privacySettings(new UserSetting.PrivacySettings())
                .syncSettings(new UserSetting.SyncSettings())
                .appearanceSettings(new UserSetting.AppearanceSettings())
                .messageSettings(new UserSetting.MessageSettings())
                .notificationSettings(new UserSetting.NotificationSettings())
                .utilitiesSettings(new UserSetting.UtilitiesSettings())
                .build();
    }
